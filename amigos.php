<?php
	...
	$pubs = mysqli_query($conexao, "SELECT * FROM amizades WHERE de='$login_cookie' or para='$login_cookie' ORDER BY id desc");
?>
<html>
<header>
	<style type="text/css">
	...
	<?php
		while ($pub=mysqli_fetch_assoc($pubs)) {
			if ($pub['de']==$login_cookie) {
				$para = $pub['para'];
				$info = mysqli_query($conexao, "SELECT * FROM users WHERE email = '$para'");
				...
				$de = $pub['de'];
				$info = mysqli_query($conexao, "SELECT * FROM users WHERE email = '$de'");
				$amigoinfo = mysqli_fetch_assoc($info);
				echo '<div class="pub">
					<p>Amigos desde '.$pub["data"].'</p>
					<span><a href="profile.php?id='.$amigoinfo['id'].'">'.$amigoinfo['nome'].'</a></span><br />
