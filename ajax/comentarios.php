...
$pubs = mysqli_query($conexao, "SELECT * FROM comentarios WHERE post='$id'");

if (isset($_POST['publish'])) {
	$texto = $_POST["texto"];
	$hoje = date("Y-m-d");

	$post = mysqli_query($conexao, "SELECT * FROM pubs WHERE id='$id'");
	...
	if ($texto == "") {
		...
		if ($data) {
			$not = mysqli_query($conexao, "INSERT INTO notificacoes (`userde`,`userpara`,`tipo`,`post`,`data`) VALUES ('$login_cookie','$userinfo','2','$id','$hoje')");
